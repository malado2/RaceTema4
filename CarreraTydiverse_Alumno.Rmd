...
library(pacman)
p_load(dplyr,lubridate,tidyr, plotly, ggplot2)
...
```{r}
#Vamos a almacenar el resultado (los datos de 'mydf') en una nueva variable 
#llamada 'cran'
cran <- as_tibble(mydf)
#A continuación eliminamos el data frame llamado 'mydf'
rm(mydf)
...
```{r}
#Escribimos 'cran' para visulaizar los resultados.
cran 
glimpse(cran) #Esta funcion nos muestra las columnas de nuestro data frame.
...
```{r}
cran[, c("ip_id", "package", "country")] #Seleccionamos unicamente
#esas tres columnas
```
5. Elige todas las variables entre `r_arch y country` 
```{r}
cran%>%select(r_arch:country) #De esta manera estamos seleccionando las columnas:
#r_arch, r_os, package, version y country.
```
...
```{r}
cran%>%select(-(date:size)) #Asi seleccionamos todas las columnas menos 'time' que es 
#la que esta entre data y size.
...
```{r}
cran2[order(cran2$country, cran2$r_version, cran2$ip_id), 
...
```{r}
avg_bytes <- summarise(cran3, avg_bytes = mean(correct_size))
avg_bytes
```
# Agrupamientos
...
by_package <- group_by(cran3, package)
by_package
```
